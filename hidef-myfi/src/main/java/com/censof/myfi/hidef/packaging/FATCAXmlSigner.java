...
public class FATCAXmlSigner implements ISigner {
	...
	protected final String xmlTagReferenceUriValue = "CTS";
	...
	public X509Certificate receiverPublicCert = null;

	// <Object Id="FATCA">[payload]</Object> or
	// <Object><SignatureProperties><SignatureProperty
	// Id="FATCA">[payload]</SignatureProperty></SignatureProperties></Object> or
	// <Object><SignatureProperties Id="FATCA"><SignatureProperty
	// Target="#SignatureId">[payload]</SignatureProperty></SignatureProperties></Object>
	public enum SigRefIdPos {
		Object, SignatureProperty, SignatureProperties
	}

	// transformation/canonicalization to use for signing. transformation affects
	// digest value and thus signature value
	// Inclusive: <Transforms><Transform
	// Algorithm="http://www.w3.org/TR/2001/REC-xml-c14n-20010315"/></Transforms>
	// InclusiveWithComments: <Transforms><Transform
	// Algorithm="http://www.w3.org/TR/2001/REC-xml-c14n-20010315#WithComments"/></Transforms>
	// Exclusive: <Transforms><Transform
	// Algorithm="http://www.w3.org/2001/10/xml-exc-c14n#"/></Transforms>
	// ExclusiveWithComments: <Transforms><Transform
	// Algorithm="http://www.w3.org/2001/10/xml-exc-c14n#WithComments"/></Transforms>
	public enum SigXmlTransform {
		Inclusive, InclusiveWithComments, Exclusive, ExclusiveWithComments, None
	};

	public enum SigDocType {
		XML, WRAPPEDXML, // binary (base64 encoded) and text files are wrapped in xml before signing
	...
	// base64 binary and text are wrapped in xml before signing
	// returns start and end wrapper tags. Sample:<Wrapper
	// xmlns="urn:xmpp:xml-element"> and </Wrapper>
	protected byte[][] getWrapperTags() throws Exception {
		String ns = myThreadSafeData.getWrapperNS(), prefix = myThreadSafeData.getWrapperPrefix(), xsi = null,
				xsiSchemaLoc = null;
		...
		if ("".equals(prefix)) {
			...
			startTag = "<Wrapper xmlns=\"" + ns + "\""
					+ (xsi == null ? "" : " " + xsi + (xsiSchemaLoc == null ? "" : " " + xsiSchemaLoc)) + ">";
			endTag = "</Wrapper>";
		} else {
			// <xyz:Wrapper xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance"
			// xmlns:xyz="urn:xmpp:xml-element" xsi:schemaLocation="urn:xmpp:xml-element
			// FATCA-IDES-FileWrapper-1.1.xsd">
	...
	// used with streaming based signing. XML is parsed and message digest is
	// calculated.
	// this methods gets a xml start tag or end tag or characters/value and it first
	// transforms data and updates digest
	protected void processXmlFrag(int type, String tag, String val, MessageDigest messageDigest,
			Canonicalizer canonicalizer, DocumentBuilder docBuilderNSTrue, String wrapperPrefix, String wrapperNSUri)
			throws Exception {
		...
		String wrapperNS = wrapperPrefix == null ? (wrapperNSUri == null ? null : "xmlns=\"" + wrapperNSUri + "\"")
				: (wrapperNSUri == null ? null : "xmlns:" + wrapperPrefix + "=\"" + wrapperNSUri + "\"");
	...
	// for an binary file, it creates base64 encoded file. if 'isCalcDigest' is
	// true, this also calculate digest
	// digest calc is useful for while signing with Base64 transformation (not used
	// in fatca which wraps base64 binary in xml and sign xml)
	...
	// fatca uses enveloping signing where payload is embedded within <Signature..>.
	// payload may be enclosed in different ways:
	...
	protected byte[][] getSigRefIdPosTags() {
		...
		case SignatureProperties:
			if ("".equals(prefix)) {
				tags[0] = ("<SignatureProperties xmlns=\"" + XMLSignature.XMLNS + "\" Id=\"" + xmlTagReferenceUriValue
						+ "\"><SignatureProperty Target=\"#" + xmlTagSignatureIdValue + "\">").getBytes();
				tags[1] = "</SignatureProperty></SignatureProperties>".getBytes();
			} else {
				tags[0] = ("<" + prefix + ":SignatureProperties xmlns" + ":" + prefix + "=\"" + XMLSignature.XMLNS
						+ "\" Id=\"" + xmlTagReferenceUriValue + "\"><" + prefix + ":SignatureProperty Target=\"#"
						+ xmlTagSignatureIdValue + "\">").getBytes();
				tags[1] = ("</" + prefix + ":SignatureProperty></" + prefix + ":SignatureProperties>").getBytes();
			}
			break;
		case SignatureProperty:
			if ("".equals(prefix)) {
				tags[0] = ("<SignatureProperty xmlns=\"" + XMLSignature.XMLNS + "\" Id=\"" + xmlTagReferenceUriValue
						+ "\" Target=\"#" + xmlTagSignatureIdValue + "\">").getBytes();
				tags[1] = "</SignatureProperty>".getBytes();
			} else {
				tags[0] = ("<" + prefix + ":SignatureProperty xmlns" + ":" + prefix + "=\"" + XMLSignature.XMLNS
						+ "\" Id=\"" + xmlTagReferenceUriValue + "\" Target=\"#" + xmlTagSignatureIdValue + "\">")
								.getBytes();
		...
		default:
			if ("".equals(prefix)) {
				tags[0] = ("<Object xmlns=\"" + XMLSignature.XMLNS + "\" Id=\"" + xmlTagReferenceUriValue + "\">")
						.getBytes();
				tags[1] = "</Object>".getBytes();
			} else {
				tags[0] = ("<" + prefix + ":Object xmlns" + ":" + prefix + "=\"" + XMLSignature.XMLNS + "\" Id=\""
						+ xmlTagReferenceUriValue + "\">").getBytes();
	...
	// used with streaming based signing. XML is parsed and message digest is
	// calculated.
	// this method parses a xml, reads a chunk of doc, say 8192 or more bytes, and
	// transforms xml and updates digest
	protected ParseXmlReturn calcCanonicalizedXmlMsgDigestByParsingDocChunk(String infile, MessageDigest messageDigest,
		...
		logger.debug("--> calcCanonicalizedXmlMsgDigestByParsingDocChunk(). infile=" + infile + ", sigXmlTransform="
				+ sigXmlTransform);
		...
		// hashNS is created at start elem and destroyed at end elem. It contains all
		// inherited NS and new ones, if defined
		// hashChunkNS is created at start elem of a chunk and destroyed once chunk is
		// processed. It contains only new NS defined in the chunk
		// for chunk processing, we need to add prefix start tags with those NS exists
		// in hashNS but not in hashChunkNS
		HashMap<String, String> hashNS = new HashMap<String, String>(), hashChunkNS = new HashMap<String, String>();
		// stackTag pushes tag in start elem and pop/destroy in end elem.
		// stackChunkStartTag contains start tag of chunk, popped in end tag
		// stackChunkEndTag contains end tags in chunk for missing start tag in chunk.
		// For chunk processing, for each stackChunkEndTag elements, a start tag prefix
		// is created
		Stack<String> stackTag = new Stack<String>(), stackChunkStartTag = new Stack<String>(),
				stackChunkEndTag = new Stack<String>();
		// stackNS stacks hashNS which is created start of elem and popped/destroyed at
		// end elem
		...
		try {
			...
			reader = XMLInputFactory.newInstance().createXMLStreamReader(new FileInputStream(new File(infile)));
			while (!isEndDoc) {
				sbChunk.setLength(0);
				// logger.debug("Exception =============> 2 "+reader.getEventType());
				...
				case XMLStreamConstants.START_ELEMENT:
					...
					//logger.debug("prefix here===>"+prefix);
					localname = reader.getLocalName();
					/*
					 * qnameS = ((prefix == "" || prefix.isEmpty() || prefix == null ||
					 * prefix.equalsIgnoreCase("null") ) ? "" : prefix + ":") + localname;
					 */
					if (prefix != null && !prefix.isEmpty() && !prefix.equalsIgnoreCase("null")) {
						qnameS = prefix + ":" + localname;
					} else {
						qnameS = localname;
					}
					...
					if (nscount > 0) {
						if (!stackNS.empty()) {
							...
							HashMap<String, String> tmpHashNS = (HashMap<String, String>) stackNS.peek().clone(); // clone
																													// parent
																													// hashNS
																													// to
																													// inherit
...
//					logger.info("tmpS ===>"+tmpS);
				...
				case XMLStreamConstants.END_ELEMENT:
					...
					if (parseBuf.length() > minChunkSize) {
						...
						// canonicalization may modify startPrefixTags so note startPrefixTagCount to
						...
						for (int i = 0; i < count; i++) {
							...
							// if stackChunkStartTag oldest entry matches stackTag current entry then no
							// more need for stackTag entries for startPrefixTags as start tag exists in
							// chunk
							if (isStartPrefixTag && !stackChunkStartTag.empty()
									&& localname.equals(stackChunkStartTag.get(0)))
						...
						// add missing start tag in chunk whose end tag exists in chunk with no
						// corresponding start tag
						boolean isFirstIteration = true;
						while (!stackChunkEndTag.empty()) {
							localname = stackChunkEndTag.pop();
							// "".equals(startPrefixTags) checks is to determine whether sbns has been
							// considered or not
						...
						logger.info("Exception =============> modifiedval ==> "+modifiedval);
						...
						if (endSuffixTags.length() > 0)
							digestval = digestval.substring(0, digestval.length() - endSuffixTags.length()); // simply
																												// drop
																												// endSuffixTags
																												// -
																												// they
			...
			logger.debug("Exception =============>" + errors.toString());
	...
	// used with streaming based signing. XML is parsed and message digest is
	// calculated.
	// this method parses a xml, and for every start tag or end tag or
	// characters/value invokes processXmlFrag which transforms data and updates
	// digest
	protected ParseXmlReturn calcCanonicalizedXmlMsgDigestByParsingDocNoChunk(String infile,
			MessageDigest messageDigest, SigXmlTransform sigXmlTransform) throws Exception {
		logger.debug("--> calcCanonicalizedXmlMsgDigestByParsingDocNoChunk(). infile=" + infile + ", sigXmlTransform="
				+ sigXmlTransform);
			...
			while (reader != null && reader.hasNext()) {
				...
				case XMLStreamConstants.START_ELEMENT:
					...
					for (int i = 0; i < count; i++) {
						...
						localname = reader.getAttributeLocalName(i);
						prefix = reader.getAttributePrefix(i);
						parseBuf.append(" " + ("".equals(prefix) ? localname : (prefix + ":" + localname)) + "=\""
								+ tmpS + "\"");
					}
					parseBuf.append(">");
					processXmlFrag(STARTTAG, qnameS, parseBuf.toString(), messageDigest, canonicalizer,
							docBuilderNSTrue, wrapperPrefix, wrapperNSUri);
					break;
				case XMLStreamConstants.CHARACTERS:
					tmpS = reader.getText();
					tmpS = tmpS.replace("&", "&#x26;").replace("\"", "&#x22;").replace("'", "&#x27;")
				...
				case XMLStreamConstants.END_ELEMENT:
					...
					parseBuf.append(localname);
					parseBuf.append('>');
					processXmlFrag(ENDTAG, qnameS, parseBuf.toString(), messageDigest, canonicalizer, docBuilderNSTrue,
							wrapperPrefix, wrapperNSUri);
	...
	// calculate message digest as is - no transformation. used to sign wrapped
	// base64 binary and wrapped text document
	...
	// not used in fatca. this calculate message digest of a text file - no
	// transformation
	...
	// streaming signature uses this to create blank document with all signature
	// elements, then update <DigestValue>
	// and <SignedInfo> which is then used to calculate signature value which is
	// then used in <SignatureValue>
	...
	// signs a xml file using JDK signature apis (JDK only supports DOM based
	// signing api - so file size, to sign, is limited by heap)
	protected Document createSignedDOMDoc(String infile, PrivateKey sigkey, X509Certificate sigPubCert,
			SigDocType sigDocType, SigXmlTransform sigXmlTransform) throws Exception {
		logger.debug("--> createSignedDOMDoc(). infile=" + infile + ", sigDocType=" + sigDocType + ", sigXmlTransform="
				+ sigXmlTransform);
		...
		try {
			...
			if (sigDocType == SigDocType.XML) {
				if (sigXmlTransform != SigXmlTransform.None)
					transforms = Collections.singletonList(xmlSigFactory
							.newTransform(getCanonicalizationMethod(sigXmlTransform), (TransformParameterSpec) null));
			...
			switch (myThreadSafeData.getSigRefIdPos()) {
			case Object:
				xmlobj = xmlSigFactory.newXMLObject(Collections.singletonList(content), xmlTagReferenceUriValue, null,
						null);
				break;
			case SignatureProperty:
				sigProp = xmlSigFactory.newSignatureProperty(Collections.singletonList(content),
						"#" + xmlTagSignatureIdValue, xmlTagReferenceUriValue);
			...
			case SignatureProperties:
				sigProp = xmlSigFactory.newSignatureProperty(Collections.singletonList(content),
						"#" + xmlTagSignatureIdValue, null);
				sigProps = xmlSigFactory.newSignatureProperties(Collections.singletonList(sigProp),
						xmlTagReferenceUriValue);
				xmlobj = xmlSigFactory.newXMLObject(Collections.singletonList(sigProps), null, null, null);
				break;
			}
			logger.debug("Near creating xml info======>");
			List<XMLObject> xmlObjs = Collections.singletonList(xmlobj);
			Reference sigref = xmlSigFactory.newReference("#" + xmlTagReferenceUriValue,
					xmlSigFactory.newDigestMethod(xmlTagDigestMethodAlgoValue, null), transforms, null, null);
			...
			logger.debug("Near creating key info======>");
			if (sigPubCert != null) {
				KeyInfoFactory keyInfoFactory = xmlSigFactory.getKeyInfoFactory();
                List<Object> x509Content = new ArrayList<Object>();
                logger.debug("The content of ky info is=====>"+sigPubCert.getSubjectX500Principal().getName());
                x509Content.add(sigPubCert.getSubjectX500Principal().getName());
                x509Content.add(sigPubCert);
                X509Data kv = keyInfoFactory.newX509Data(x509Content);
				keyInfo = keyInfoFactory.newKeyInfo(Collections.singletonList(kv));
			}else {
				logger.debug("Signed public cert is null");
			}
			XMLSignature signature = xmlSigFactory.newXMLSignature(signedInfo, keyInfo, xmlObjs, xmlTagSignatureIdValue,
					null);
			doc = docBuilderNSTrue.newDocument();
	...
	// JDK only supports DOM based signing api - so file size, to sign, is limited
	// by heap. this method calculate digest by reading a file stream
	// (do not need to load entire file as required in DOM) - once digest is
	// calculated using right transformation, rest is easy -
	// create blank signed document, then update <DigestValue> and <SignedInfo>
	// which is then used to calculate signature value
	// which is then populated in <SignatureValue>. eventually write enveloping
	// signature and payload in output file
	protected boolean signFileStreaming(String infile, String outfile, PrivateKey sigkey, X509Certificate sigPubCert,
			SigDocType sigDocType, SigXmlTransform sigXmlTransform) throws Exception {
		logger.debug("--> signFileStreaming(). infile=" + infile + ", outfile=" + outfile + ", sigDocType=" + sigDocType
				+ ", sigXmlTransform=" + sigXmlTransform);
		boolean ret = false;
		...
		try {
			...
			case XML:
				...
				if (myThreadSafeData.isXmlChunkStreaming())
					parseXmlRet = calcCanonicalizedXmlMsgDigestByParsingDocChunk(infile, messageDigest,
							sigXmlTransform);
				else
					parseXmlRet = calcCanonicalizedXmlMsgDigestByParsingDocNoChunk(infile, messageDigest,
							sigXmlTransform);
			...
			Document doc = createBlankSignedDOMDocForStreamingSignature(sigkey, sigPubCert, sigDocType,
					sigXmlTransform);
			...
			logger.debug("===========Adding in the ky info=======");
			nodeList = doc.getElementsByTagNameNS(XMLSignature.XMLNS, "KeyInfo");
			if (nodeList.getLength() > 0) {
				node = nodeList.item(0);
				node = node.getFirstChild();
	...
	protected boolean signFile(String infile, String outfile, PrivateKey sigkey, X509Certificate sigPubCert,
			SigDocType sigDocType, SigXmlTransform sigXmlTransform) throws Exception {
		logger.debug("--> signFile(). infile=" + infile + ", outfile=" + outfile + ", sigDocType=" + sigDocType
				+ ", sigXmlTransform=" + sigXmlTransform);
		BufferedOutputStream bos = null;
		boolean ret = false;
		try {
	...
	protected boolean wrapBinaryFileInXmlAndSign(String infile, String outfile, PrivateKey sigkey,
		...
		logger.debug(
				"--> wrapBinaryFileInXmlAndSign(). infile=" + infile + ", outfile=" + outfile + ", isDOM=" + isDOM);
		boolean flag = false;
		...
		bos.close();
		if (isDOM)
			flag = signFile(wrappedBase64BinaryFile, outfile, sigkey, sigPubCert, SigDocType.WRAPPEDXML,
					SigXmlTransform.None);
		else
			flag = signFileStreaming(wrappedBase64BinaryFile, outfile, sigkey, sigPubCert, SigDocType.WRAPPEDXML,
					SigXmlTransform.None);
	...
	protected boolean wrapTextFileInXmlAndSign(String infile, String outfile, PrivateKey sigkey,
		...
		if (isDOM)
			...
			flag = signFileStreaming(wrappedTextFile, outfile, sigkey, sigPubCert, SigDocType.WRAPPEDXML,
					SigXmlTransform.None);
	...
	protected boolean signFileNoWrap(String infile, String outfile, PrivateKey sigkey, X509Certificate sugPubCert,
		...
		return signFileNoWrap(infile, outfile, sigkey, sugPubCert, isDom, sigDocType,
				myThreadSafeData.getSigXmlTransform());
	}

	protected boolean signFileNoWrap(String infile, String outfile, PrivateKey sigkey, X509Certificate sugPubCert,
			boolean isDom, SigDocType sigDocType, SigXmlTransform sigXmlTransform) throws Exception {
		logger.debug("--> signFileNoWrap(). infile=" + infile + ", outfile=" + outfile + ", isDom=" + isDom
				+ ", sigDocType=" + sigDocType + ", sigXmlTransform=" + sigXmlTransform);
		boolean flag = false;
		if (isDom)
	...
	// <xyz:Wrapper ...xsi:schemaLocation="urn:xmpp:xml-element
	// FATCA-IDES-FileWrapper-1.1.xsd">
	// 'schemaLocation' value - urn:xmpp:xml-element FATCA-IDES-FileWrapper-1.1.xsd
	// in this example
	// no need to use
	...
	public boolean signXmlFileStreaming(String infile, String outfile, PrivateKey sigkey, X509Certificate sugPubCert,X509Certificate receiverPblicCert)
			throws Exception {
		boolean isDom = false;
		receiverPublicCert = receiverPblicCert;
		return signFileNoWrap(infile, outfile, sigkey, sugPubCert, isDom, SigDocType.XML,
				myThreadSafeData.getSigXmlTransform());
	}

	public boolean wrapBinaryFileInXmlAndSignStreaming(String infile, String outfile, PrivateKey sigkey,
			X509Certificate sugPubCert) throws Exception {
		boolean isDom = false;
		return wrapBinaryFileInXmlAndSign(infile, outfile, sigkey, sugPubCert, isDom);
	}

	public boolean wrapTextFileInXmlAndSignStreaming(String infile, String outfile, PrivateKey sigkey,
			X509Certificate sugPubCert) throws Exception {
		boolean isDom = false;
	...
	public boolean signXmlFile(String infile, String outfile, PrivateKey sigkey, X509Certificate sugPubCert)
			throws Exception {
		return signFileNoWrap(infile, outfile, sigkey, sugPubCert, true, SigDocType.XML);
	}

	// wrap text in xml and sign - dom based
	public boolean wrapTextFileInXmlAndSign(String infile, String outfile, PrivateKey sigkey,
			X509Certificate sugPubCert) throws Exception {
		boolean isDom = true;
		return wrapTextFileInXmlAndSign(infile, outfile, sigkey, sugPubCert, isDom);
	}
	...
	public boolean wrapBinaryFileInXmlAndSign(String infile, String outfile, PrivateKey sigkey,
			X509Certificate sugPubCert) throws Exception {
		boolean isDom = true;
		return wrapBinaryFileInXmlAndSign(infile, outfile, sigkey, sugPubCert, isDom);
	}
	...
	public void setSigRefIdPos(String sigRefIdPos) throws Exception {
		if ("Object".equalsIgnoreCase(sigRefIdPos))
			...
			throw new Exception("invalid sigRefIdPos=" + sigRefIdPos
					+ ". Valid values are Object|SignatureProperty|SignatureProperties");
	}
	...
	public void setSigXmlTransform(String sigXmlTransform) throws Exception {
		if ("Inclusive".equalsIgnoreCase(sigXmlTransform))
			...
			throw new Exception("invalid sigXmlTransform=" + sigXmlTransform
	...
	// chunk size if XmlChunkStreaming is true - default is 8092. XmlChunk is used
	// with streaming based signing to calculate message digest
	public int getXmlChunkStreamingSize() {
	...
	// not used in FATCA
	public boolean signTextFile(String infile, String outfile, PrivateKey sigkey, X509Certificate sugPubCert)
			throws Exception {
		return signFileNoWrap(infile, outfile, sigkey, sugPubCert, true, SigDocType.TEXT, SigXmlTransform.None);
	}

	// not used in FATCA
	public boolean signBinaryFile(String infile, String outfile, PrivateKey sigkey, X509Certificate sugPubCert)
			throws Exception {
		return signFileNoWrap(infile, outfile, sigkey, sugPubCert, true, SigDocType.BINARY, SigXmlTransform.None);
	}

	// not used in FATCA
	public boolean signTextFileStreaming(String infile, String outfile, PrivateKey sigkey, X509Certificate sugPubCert)
			throws Exception {
		boolean isDom = false;
	...
	// not used in FATCA
	public boolean signBinaryFileStreaming(String infile, String outfile, PrivateKey sigkey, X509Certificate sugPubCert)
			throws Exception {
		boolean isDom = false;
